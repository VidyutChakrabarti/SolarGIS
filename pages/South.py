...
if get_redis_state("bbox_coords") is None:
    initial_bbox_coords = [[79.070953, 21.153387], [79.070953, 21.153637], [79.0712, 21.153637], [79.0712, 21.153387]]
    set_redis_state("bbox_coords", initial_bbox_coords)
...
if get_redis_state("dt3") is None:
    set_redis_state("dt3", None)
...
with c1:
    image = Image.open(get_redis_state("upis")[2])   
    ...
    if st.button("Select Object", key="submit_rect", disabled=(not get_redis_state("bbox_confirmed") or get_redis_state("drawing_mode") != "Rectangle")):
        if canvas_result.json_data:
            ...
            set_redis_state("drawing_mode", "Line")  
            st.rerun()
...
with st.form(key='df'):
    ...
    if next_page:
        ...
        set_redis_state("dt3", alter_df(annotations_df))
        set_redis_state("new_box", None)
        set_redis_state("annotations", [])  
        reset_redis_state()  
        switch_page('East')
